#!/usr/bin/env python3
# gemma3_schmuck_finetune_A100_multistage.py
...
class SchmuckGemma3Finetune:
    def __init__(self, model_path: str):
        ...
        # Optional tracking of best model
...
Return ONLY the JSON object, properly formatted."""
    ...
    def compute_metrics_for_trainer(self, eval_preds, compute_result: bool = True):
        ...
        Uses CER as main metric. Trainer will log 'eval_cer'.
        """
    ...
    def train_model(
        ...
        output_dir: str,
        num_epochs: int = 15,
        batch_size: int = 2,   # A100 can handle 2
        learning_rate: float = 3e-5,
        stage1_epochs: int = 5,
    ):
        """
        Multi-stage training:
        - Stage 1: warm-up, no eval, teacher forcing
        - Stage 2: main training, eval each epoch, best model by lowest eval_cer
        """
        ...
        stage1_args = SFTConfig(
            ...
            learning_rate=learning_rate * 3.0,  # slightly higher LR for warmup
            logging_steps=5,
            eval_strategy="no",
            save_strategy="no",
            save_total_limit=1,
        ...
        print("Starting Stage 2 training with CER-based evaluation (A100-optimized)...")
        ...
        # Trainer has already loaded best eval_cer checkpoint into its model
    ...
    def evaluate_on_test_set(
        ...
        """Evaluate with aggressive memory management and smaller chunks using best-CER model in memory."""
...
Return ONLY the JSON object, properly formatted."""
            ...
            for i, test_item in enumerate(chunk_data):
                ...
                try:
                    ...
                    if image is not None:
                        image.close()
                        del image
                    for var in ["inputs", "outputs", "generated_ids_trimmed"]:
                        if var in locals():
                            del locals()[var]
                    torch.cuda.empty_cache()
                    gc.collect()
    ...
    def calculate_cer_statistics(self, all_cer_scores: List[float]) -> Dict[str, Any]:
    ...
    def save_cer_results(self, cer_stats: Dict[str, Any], cer_file: str, num_predictions: int):
...
def main():
    """Main function - A100 optimized with multi-stage training + CER best model."""

    base_checkpoint_dir = (
        "/home/vault/iwi5/iwi5298h/models_image_text/gemma/multi/schmuck"
    )
    ...
    print("\n" + "=" * 60)
    print("GEMMA-3 SCHMUCK - A100 OPTIMIZED MULTI-STAGE")
    print("=" * 60)

    finetuner = SchmuckGemma3Finetune(model_path=local_model_path)

    print("\n" + "=" * 60)
    print("STARTING MULTI-STAGE TRAINING (A100-OPTIMIZED)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("STARTING EVALUATION ON TEST.JSONL (BEST-CER MODEL)")
    print("=" * 60)
