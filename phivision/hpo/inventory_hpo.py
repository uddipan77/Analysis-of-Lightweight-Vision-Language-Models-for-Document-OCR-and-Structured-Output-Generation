#!/usr/bin/env python3
# phi_vision_inventory_hpo_fast.py
#
# Fast Hyperparameter Optimization for Phi-3.5-Vision on INVENTORY dataset.
# Key speed-ups:
#   - Fixed num_epochs (8) for all trials (tuned around your best config)
#   - NO per-epoch CER callback (no generation during training)
#   - Single CER evaluation per trial on a small random val subset
#   - No augmentation for HPO (to keep trials cheap & consistent)
#   - Gradient checkpointing ENABLED (like staircase HPO, to fit in 32GB)
...
# ============================================================================
# Configuration - Inventory dataset & HPO
# ============================================================================
...
CONFIG: Dict[str, Any] = {
    ...
    # Centered around your current good config
    "num_epochs": 5,
    "batch_size": 1,
    ...
    "max_seq_length": 2048,          # enforce same as staircase HPO
    "processor_num_crops": 8,        # use fewer crops for HPO
    # =================================================

    # Optuna
    "optuna_n_trials": 25,  # total desired COMPLETE trials across all runs
    "optuna_db_path": "/home/hpc/iwi5/iwi5298h/Uddipan-Thesis/vlmmodels.db",
    "optuna_study_name": "phi_new_inventory_dataset",

    # Base directory for HPO outputs; a timestamped run folder is created inside
    "hpo_output_base_dir": "/home/vault/iwi5/iwi5298h/models_image_text/phi/hpo/new_inven",
...
# ============================================================================
# Instruction Prompt for INVENTORY Dataset
# ============================================================================
...
class DocumentImageAugmenter:
    """Augmentation for historical inventory documents."""
...
class InventoryDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply chat template
        # ========= CHANGED: enforce max_length + truncation =========
...
def evaluate_cer_on_validation(
    ...
    """
    Run generation on a FIXED subset of validation data and
    compute average CER. This is the HPO objective.
    Uses deterministic subset selection (sorted by image_name) to ensure
    the same samples are used across all trials.
    """
    print("\n" + "=" * 80)
    print("🔍 RUNNING CER EVALUATION ON INVENTORY VALIDATION SET (HPO OBJECTIVE)")
    print("=" * 80)

    device = next(model.parameters()).device
    model.eval()

    # Deterministic subset selection: sort by image_name, take first max_samples
    # This ensures SAME samples are used across all trials
    ...
    print(f"   • Using FIXED validation subset of {len(val_subset)} samples for CER")
...
def train_one_config(
    ...
    """
    print("\n" + "=" * 80)
    print("PHI-3.5-VISION FINE-TUNING (HPO TRIAL) - INVENTORY DATASET")
    ...
    # ========= CHANGED: enable gradient checkpointing (like staircase) =========
    ...
    # Train
    print("🚀 Starting QLoRA training on INVENTORY dataset (HPO trial)...\n")
    print("=" * 80)
    trainer.train()
    print("=" * 80)
...
def run_optuna_hpo_only(
    ...
    print("\n" + "=" * 80)
    print("🔎 Starting / Resuming Optuna HPO (Phi-3.5-Vision, INVENTORY, FAST MODE)")
    print("=" * 80)
    ...
    if remaining > 0:
        ...
        def objective(trial: optuna.trial.Trial):
            ...
            # Fixed epochs & batch size for speed (centered at your chosen values)
    ...
    # After HPO, save best hyperparams to files (inside this run_dir)
    best_trial = study.best_trial
    print("\n" + "=" * 80)
    print("🏆 Optuna HPO finished / resumed (Phi-3.5-Vision, INVENTORY, FAST MODE)")
    print("=" * 80)
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("Optuna HPO Summary - PHI-3.5-Vision INVENTORY OCR (FAST MODE)\n")
        f.write("=" * 80 + "\n\n")
    ...
    print("\n🎉 HPO run complete. You can now copy these hyperparameters into your main INVENTORY fine-tuning script.\n")
...
def main():
    ...
    processor = AutoProcessor.from_pretrained(
        CONFIG["model_path"],
        trust_remote_code=True,
        num_crops=CONFIG["processor_num_crops"],   # CHANGED: fewer crops for HPO
    )
    ...
    print("   📁 Validation dataset (for length info only):")
    _ = InventoryDataset(
        CONFIG["val_jsonl"],
        CONFIG["images_dir"],
        processor,
