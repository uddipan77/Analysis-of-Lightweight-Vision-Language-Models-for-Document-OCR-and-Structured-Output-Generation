#!/usr/bin/env python3
# phi_3_5_vision_schmuck_finetune_qlora_multistage.py
# Multi-stage memory-optimized fine-tuning with BitsAndBytes QLoRA 4-bit for Schmuck dataset
...
CONFIG = {
    ...
    # This will be overridden in main() with a timestamped run_dir
    "output_dir": "/home/vault/iwi5/iwi5298h/models_image_text/phi/multistage/schmuck",
    "num_epochs": 10,
    "batch_size": 1,
    ...
    # Multi-stage: number of warmup epochs
    "stage1_epochs": 3,
}
...
class SchmuckDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Teacher forcing: labels = input_ids, with prompt masked
...
# ============================================================================ #
# CER Validation Callback (Stage 2) – saves best checkpoint by CER            #
# ============================================================================ #
...
def main():
    print("\n" + "=" * 80)
    print("PHI-3.5-VISION FINE-TUNING - SCHMUCK DATASET (MULTI-STAGE)")
    print("BitsAndBytes QLoRA 4-bit (Memory-Optimized)")
    print("=" * 80)

    # Create per-run directory under the multistage base
    base_dir = CONFIG["output_dir"]
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = os.path.join(base_dir, f"run_{timestamp}_multistage")
    ...
    print(f"   • Training epochs (total): {CONFIG['num_epochs']}")
    print(f"   • Stage 1 epochs (warm-up): {CONFIG['stage1_epochs']}")
    print(f"   • Stage 2 epochs (main): {max(1, CONFIG['num_epochs'] - CONFIG['stage1_epochs'])}")
    print(f"   • Batch size: {CONFIG['batch_size']}")
    print(f"   • Gradient accumulation: {CONFIG['gradient_accumulation_steps']}")
    print(f"   • Learning rate (Stage 2 base): {CONFIG['learning_rate']}")
    ...
    # Validation data for CER callback (stage 2)
    ...
    remaining_epochs = max(1, CONFIG["num_epochs"] - stage1_epochs)
    ...
    training_args_stage1 = TrainingArguments(
        ...
        learning_rate=CONFIG["learning_rate"] * 2.0,  # higher LR for warmup
        ...
        logging_dir=os.path.join(CONFIG["output_dir"], "logs_stage1"),
    ...
    if torch.cuda.is_available():
        ...
        print(f"\n{'=' * 80}")
        print("GPU INFORMATION (STAGE 1)")
        print(f"{'=' * 80}")
        print(f"   GPU: {gpu_stats.name}")
        print(f"   Max memory: {round(gpu_stats.total_memory / 1024**3, 3)} GB")
        print(f"{'=' * 80}\n")

    print("🚀 Starting Stage 1 (warm-up) training...\n")
    ...
    # ========================== #
    # STAGE 2: Main training + CER
    # ========================== #

    print("\n" + "=" * 80)
    print("STAGE 2: MAIN TRAINING WITH CER-BASED BEST MODEL")
    print("=" * 80)
    ...
    training_args_stage2 = TrainingArguments(
        ...
        eval_strategy="epoch",
        save_total_limit=2,
        load_best_model_at_end=True,          # <<< IMPORTANT FIX
        ...
        remove_unused_columns=False,
        report_to="tensorboard",
        logging_dir=os.path.join(CONFIG["output_dir"], "logs_stage2"),
        optim="paged_adamw_8bit",
    )
    ...
    cer_callback = CERValidationCallback(
        ...
        output_dir=CONFIG["output_dir"],  # best_model saved at run_dir/best_model
    )
    ...
    print("🚀 Starting Stage 2 training with CER-based validation...\n")
    trainer_stage2.train()
    print("\n✅ Stage 2 completed.\n")

    print("\n" + "=" * 80)
    print("✅ TRAINING COMPLETED (MULTI-STAGE)")
    print("=" * 80)
    ...
    # ========================== #
    # PHASE 2: Evaluation on test set with best CER model
    # ========================== #

    print("\n" + "=" * 80)
    print("PHASE 2: EVALUATION ON TEST SET (USING BEST CER MODEL)")
    print("=" * 80)
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("PHI-3.5-VISION SCHMUCK DATASET FINE-TUNING RESULTS (MULTI-STAGE)\n")
        f.write("=" * 80 + "\n\n")
