#!/usr/bin/env python3
# phi_3_5_vision_staircase_finetune_qlora_multi_stage.py
# Memory-optimized fine-tuning with BitsAndBytes QLoRA 4-bit for STAIRCASE dataset
# Multi-stage training (Stage 1 warm-up + Stage 2 CER-based training)
...
# ============================================================================
# Configuration - Staircase dataset (Multi-stage)
# ============================================================================
...
CONFIG = {
    ...
    # base dir – inside this we create stage1/, stage2/, best_model/, logs, etc.
    "output_dir": "/home/vault/iwi5/iwi5298h/models_image_text/phi/multistage/stair",
    "num_epochs": 10,           # total epochs = stage1 + stage2
    "stage1_epochs": 4,         # warm-up stage (you can tweak)
    "batch_size": 1,
    ...
    "lora_dropout": 0.1,
    "max_grad_norm": 1.0,
    "early_stopping_patience": 3,  # not used directly, just documented
    ...
    def on_evaluate(self, args, state, control, model, metrics=None, **kwargs):
        ...
        if val_cer < self.best_cer:
            ...
            try:
                ...
                print(
                    "   ✅ Model saved (tokenizer will be loaded from original path)"
                )
...
# ============================================================================
# Main Fine-tuning Function (Multi-stage)
# ============================================================================


def main():
    print("\n" + "=" * 80)
    print("PHI-3.5-VISION FINE-TUNING - STAIRCASE DATASET (MULTI-STAGE)")
    print("BitsAndBytes QLoRA 4-bit (Memory-Optimized)")
    print("Stage 1: warm-up   | Stage 2: CER-based best model")
    print("=" * 80)
    ...
    print(f"   • Total epochs: {total_epochs} (Stage1: {stage1_epochs}, Stage2: {stage2_epochs})")
    print(f"   • Batch size: {CONFIG['batch_size']}")
    print(f"   • Gradient accumulation: {CONFIG['gradient_accumulation_steps']}")
    print(f"   • Learning rate (base): {CONFIG['learning_rate']}")
    ...
    # ----------------------------------------------------------------------
    # STAGE 1 – Warm-up training (no eval, higher LR, no CER)
    # ----------------------------------------------------------------------
    print("\n" + "=" * 80)
    print("STAGE 1: WARM-UP TRAINING (no eval, teacher forcing)")
    print("=" * 80)

    stage1_output_dir = os.path.join(CONFIG["output_dir"], "stage1")
    ...
    stage1_args = TrainingArguments(
        ...
        learning_rate=CONFIG["learning_rate"] * 3.0,  # higher LR for warm-up
        weight_decay=CONFIG["weight_decay"],
        max_grad_norm=CONFIG["max_grad_norm"],
        warmup_ratio=0.1,
        lr_scheduler_type="cosine",
        logging_steps=10,
        save_strategy="no",      # no checkpointing in warm-up
    ...
    trainer_stage1 = Trainer(
        ...
        callbacks=[],  # no CER callback in warm-up
    )

    print("🚀 Starting Stage 1 training...\n")
    trainer_stage1.train()
    print("\n✅ Stage 1 (warm-up) completed!\n")

    # Free Stage 1 trainer
    del trainer_stage1
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    # ----------------------------------------------------------------------
    # STAGE 2 – Main training with CER-based model selection
    # ----------------------------------------------------------------------
    print("\n" + "=" * 80)
    print("STAGE 2: MAIN TRAINING (eval each epoch, CER-based best model)")
    print("=" * 80)
    ...
    cer_callback = CERValidationCallback(
        ...
        output_dir=CONFIG["output_dir"],  # best_model saved under base output_dir
    )

    stage2_output_dir = os.path.join(CONFIG["output_dir"], "stage2")
    ...
    stage2_args = TrainingArguments(
        ...
        eval_strategy="epoch",
        save_total_limit=2,
        load_best_model_at_end=False,  # CER callback controls "best" saving
    ...
    print("\n" + "=" * 80)
    print("✅ TRAINING COMPLETED (Stage 1 + Stage 2)!")
    print("=" * 80)
    print(
        f"   🎯 Best Validation CER (Stage 2): {cer_callback.best_cer:.4f} "
    ...
    # ----------------------------------------------------------------------
    # PHASE 2: EVALUATION ON TEST SET (using best CER model)
    # ----------------------------------------------------------------------
    print("\n" + "=" * 80)
    ...
    print(f"\n{'='*80}")
    print("📊 FINAL TEST RESULTS - STAIRCASE DATASET (MULTI-STAGE)")
    print(f"{'='*80}")
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("PHI-3.5-VISION STAIRCASE DATASET FINE-TUNING RESULTS (MULTI-STAGE)\n")
        f.write("=" * 80 + "\n\n")
        f.write(
            f"Best Validation CER (Stage 2): {cer_callback.best_cer:.4f} "
            f"({cer_callback.best_cer*100:.2f}%)\n"
    ...
    print("\n🎉 Multi-stage fine-tuning and evaluation complete!\n")
