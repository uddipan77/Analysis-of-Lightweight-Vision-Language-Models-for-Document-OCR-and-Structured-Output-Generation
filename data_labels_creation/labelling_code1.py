...
# Example JSON outputs to teach the model
example_outputs = [
   {
...
# System message for direct extraction
system_message = SystemMessage(
    content=(
        "You are an expert in extracting structured data from historical German inventory cards.\n"
        "Analyze the provided image and return a JSON object with these exact fields:\n"
        "- image_name (filename)\n"
        "- inventory_number (e.g., 'J=fr.Eo33')\n"
        "- heading (document title)\n"
        "- measurements: {length, breadth, depth} (in cm)\n"
        "- Material (description)\n"
        "- Fundort (origin location)\n"
        "- Fundzeit (time period if available)\n"
        "- main_text (descriptive content)\n\n"
        "Return ONLY the JSON object with no additional text or explanations."
    )
)

# Function for direct LLM processing
def process_image_direct(image_path: str) -> dict:
    ...
    for idx, example_img in enumerate(example_images):
        few_shot.append(
            HumanMessage(content=[
                ...
                {"type": "text", "text": "Extract the inventory data as JSON."}
            ])
        )
...
def main():
    ...
    out_dir = os.path.join(image_dir, "ocr_results_direct")
