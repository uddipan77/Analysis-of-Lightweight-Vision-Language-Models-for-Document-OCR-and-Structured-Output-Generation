#this is an update for schmuck dataset using donut-base-finetuned-cord-v2 - uses file_name instead of image_name
import json
import os
import torch
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="DONUT CORD-v2 OCR training for Schmuck dataset")
    ...
    # UPDATED: Timestamped output dir for schmuck dataset with CORD-v2
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    OUTPUT_DIR = f"/home/vault/iwi5/iwi5298h/models_image_text/donut/schmuck/schmuck_cord_{timestamp}"
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        lr_scheduler_type="cosine",       # ✅ Safe
        warmup_ratio=0.1,                # ✅ Safe  
        max_grad_norm=1.0,               # ✅ Safe
    )
    ...
    print("Model: donut-base-finetuned-cord-v2")
    ...
    with open(scores_path, "w") as f:
        f.write("DONUT CORD-v2 MODEL TRAINING RESULTS - SCHMUCK DATASET\n")
        f.write("="*60 + "\n")
        f.write("Model: naver-clova-ix/donut-base-finetuned-cord-v2\n")
        ...
        f.write("Generation method: model.generate() (no teacher forcing)\n")
        f.write("="*60 + "\n")
        f.write("\n".join(CER_summary) + "\n")
