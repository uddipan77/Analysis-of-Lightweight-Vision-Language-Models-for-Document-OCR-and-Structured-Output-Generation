...
CONFIG = {
    ...
    "output_root": "/home/vault/iwi5/iwi5298h/models_image_text/donut/staircase_infer_cord",
}
...
class TestDataset(Dataset):
    ...
    def format_gt(self, item: Dict) -> str:
        target = {k: v for k, v in item.items() if k != "image_name"}
        return json.dumps(target, ensure_ascii=False, separators=(",", ":"))

    def __getitem__(self, idx: int) -> Dict[str, Any]:
        item = self.data[idx]
        image_path = Path(find_image_path(item["image_name"], str(self.image_dir)))
...
def infer(model, dataset, processor, device, start_token, max_length, num_beams, batch_size):
    ...
    out = []
    for batch in tqdm(loader, desc="Infer (Donut CORD)"):
        ...
        for i in range(len(gen_ids)):
            ...
            gt = dataset.format_gt(dataset.data[idx]).replace(start_token, "").strip()
            out.append({
...
def main():
    ...
    with open(score_path, "w", encoding="utf-8") as f:
        f.write("DONUT CORD (STAIRCASE) INFERENCE RESULTS\n")
        f.write("=" * 60 + "\n")
