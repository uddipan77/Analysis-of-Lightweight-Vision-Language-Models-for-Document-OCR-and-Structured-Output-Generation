...
FastAPI backend + Gradio UI in one file.
Supports staircase, inventory, and schmuck images with a single LoRA adapter.

Docker version: paths point to /app/models/* inside the container.

Run (inside container):
    python gemma_ui_docker.py
...
# ========= PATH CONFIG (Docker paths, overridable via env vars) =========
BASE_MODEL_PATH = os.environ.get(
    "BASE_MODEL_PATH",
    "/app/models/base_model",
)
FINETUNED_ADAPTER_PATH = os.environ.get(
...
if __name__ == "__main__":
    ...
    print(f"  UI:  http://0.0.0.0:{port}/ui")
    print(f"  API: http://0.0.0.0:{port}/docs")
    print("─" * 60)
