#!/usr/bin/env python3
# florence2_staircase_lora_autoreg_valcer_tb_bestload.py
# ✅ LoRA finetuning of Florence-2-large on Staircase
# ✅ AUTOREGRESSIVE (generate-based) validation CER (same behavior as test)
...
# ✅ CER uses: GT compact JSON string vs Pred compact JSON string (if JSON parses), else GT vs raw pred string
# ✅ FIX: removed dummy key in pred normalization
...
class Florence2StaircaseTrainer:
    ...
    - generation-based autoregressive validation CER (same as test)
    - best-model saving via lowest val CER
    - guaranteed best-model reload before test
    - TensorBoard logging + dedicated output folders
    """
    ...
    def __init__(self, model_path: str, output_base_dir: str):
        ...
        self.test_metrics_path = os.path.join(self.metrics_dir, "test_cer_metrics_lora.json")

        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.torch_dtype = torch.float32

        print("=" * 60)
        print("Florence-2 Staircase LoRA Fine-tuning")
        print("=" * 60)
    ...
    def _load_model(self):
        ...
        print("⏳ Loading Florence-2-large base model...")
        ...
        # Apply LoRA (train only adapter weights by default behavior of peft;
        # but we keep base params requiring grad False unless user wants full FT)
        ...
        print(f"✅ Model loaded. Trainable params: {trainable_params:,} / {total_params:,} "
              f"({trainable_params/total_params*100:.2f}%)\n")

    def load_best_model_for_inference(self):
        """Guarantee test uses best checkpoint saved via lowest val CER."""
        ...
        # Load full model weights (PEFT adapter + base)
    ...
    def _cer_from_strings(self, gt_json_string: str, pred_text: str) -> float:
        """
        Same logic as your previous scripts:
        - if pred parses as JSON -> normalize with json_to_string(pred_json)
        - else CER against raw pred text
        """
        try:
    ...
    def evaluate_autoregressive_cer(self, eval_dataset: Dataset, batch_size: int) -> float:
        """Autoregressive validation CER via generate()."""
    ...
    def train(self, train_data: List[Dict], val_data: List[Dict], images_dir: str):
        ...
        class CustomTrainer(Trainer):
            ...
            def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
                # default eval (loss etc.) just for logging consistency
        ...
        print("=" * 60)
        print("STARTING LoRA TRAINING (AUTOREGRESSIVE VAL CER)")
        print("=" * 60 + "\n")
    ...
    def predict(self, test_data: List[Dict], images_dir: str) -> List[Dict]:
        """Autoregressive test prediction using best model (load_best_model_for_inference first)."""
        ...
        pred_path = os.path.join(self.pred_dir, "test_predictions_lora.jsonl")
