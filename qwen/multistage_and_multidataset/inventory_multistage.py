#!/usr/bin/env python3
# inventory_updated_ms_fixed.py
# Qwen2.5-VL-7B Inventory OCR fine-tuning with Unsloth
# - Multi-stage training (Stage1 warmup, Stage2 main)
# - Best model selected by lowest CER on validation subset
# - Same 4-step image preprocessing for training (collator), validation (callback), and test
...
class PreprocessUnslothVisionDataCollator:
    """
    Wrapper around UnslothVisionDataCollator that applies the same 4-step preprocessing
    on-the-fly for any image paths in batch messages.
    """
    def __init__(
        self,
        base_collator,
            ...
            for msg in msgs:
                ...
                for part in content:
                    if isinstance(part, dict) and part.get("type") == "image":
                        img_obj = part.get("image", None)
                        # Only preprocess if it's still a file path
                        if isinstance(img_obj, str) and img_obj and os.path.exists(img_obj):
                            try:
                                ...
                                # If preprocessing fails, silently keep original path
                                pass
...
class InventoryOCRFinetune:
    def __init__(self, model_path: str = "/home/vault/iwi5/iwi5298h/models/qwen7b"):
        ...
        print("Model loaded successfully with LoRA (r=16).")
    ...
    def convert_to_conversation(self, sample: Dict) -> Dict:
        ...
        if isinstance(image_path, str) and image_path and os.path.exists(image_path):
            # IMPORTANT: keep image as PATH here; collator will preprocess on-the-fly
    ...
    def prepare_training_data(self, jsonl_path: str, images_dir: str) -> List[Dict]:
        ...
        print(f"Training samples prepared: {len(converted)}")
        return converted
    ...
    def calculate_cer(self, predictions, targets):
        ...
        for pred, target in zip(predictions, targets):
            ...
            try:
                ...
                continue
        return (total_cer / valid) if valid > 0 else 1.0
    ...
    def train_model(
        ...
        print(f"Training: {len(train_dataset)} samples, Validation (raw): {len(val_raw_data)} records")

        FastVisionModel.for_training(self.model)

        best_ckpt_dir = os.path.join(output_dir, "best_model_cer")

        # Collator: Unsloth -> wrapper that preprocesses image paths into PIL images
        ...
        # ==========================================================
        print("\n" + "=" * 80)
        print("STAGE 1: WARM-UP TRAINING (no eval)")
        print("=" * 80)
        ...
        stage1_args = SFTConfig(
            ...
            # ✅ CRITICAL FIX (must match the working script)
            dataset_text_field="",
        ...
        # ✅ Force-reassign collator (SFTTrainer may override it internally)
        trainer_stage1.data_collator = data_collator
        ...
        # ==========================================================
        # STAGE 2 (main) with CER callback
        # ==========================================================
        print("\n" + "=" * 80)
        print("STAGE 2: MAIN TRAINING (eval each epoch + CER callback)")
        print("=" * 80)

        stage2_epochs = max(1, num_epochs - stage1_epochs)
        print(f"Total epochs: {num_epochs} -> Stage1: {stage1_epochs}, Stage2: {stage2_epochs}")
        ...
        # ✅ Force-reassign collator (SFTTrainer may override it internally)
        trainer_stage2.data_collator = data_collator

        print("🚀 Starting Stage 2 training...\n")
        trainer_stage2.train()

        # Load best-by-CER checkpoint produced by callback
        if cer_callback.best_state_path is not None:
            ...
            if missing_keys:
                print(f"[WARN] Missing keys when loading best CER weights ({len(missing_keys)}). Showing first 10:")
                print(missing_keys[:10])
            if unexpected_keys:
                print(f"[WARN] Unexpected keys when loading best CER weights ({len(unexpected_keys)}). Showing first 10:")
                print(unexpected_keys[:10])

            trainer_stage2.model = self.model
            print(f"✅ Restored best validation CER = {cer_callback.best_cer:.4f} ({cer_callback.best_cer*100:.2f}%)")
        else:
            print("WARNING: CER callback did not save any best model; using final Stage 2 weights.")
    ...
    def evaluate_on_split(self, jsonl_path: str, images_dir: str, output_dir: str, split_name: str) -> Dict:
        ...
        with open(jsonl_path, "r", encoding="utf-8") as f:
            for idx, line in enumerate(f):
                ...
                try:
                    # ✅ SAME 4-step preprocessing for test
                    ...
                    pred_str = self.json_to_string_no_sort(predicted_json)

                    cer_score = jiwer.cer(gt_str, pred_str)
    ...
    def calculate_cer_statistics(self, all_cer_scores: List[float]) -> Dict:
        if not all_cer_scores:
            return {}
        avg = float(sum(all_cer_scores) / len(all_cer_scores))
        return {
            ...
            "perfect_matches": int(sum(1 for x in all_cer_scores if x == 0.0)),
        }
    ...
    def save_model(self, trainer, output_dir: str):
        ...
        print("Saved model (HF + merged_16bit + lora).")


# ======================================================================
# CER Validation Callback (same 4-step preprocessing)
# ======================================================================
...
class InventoryCERCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, model, tokenizer=None, metrics=None, **kwargs):
        ...
        for i, val_item in enumerate(self.val_data):
            ...
            try:
                # ✅ SAME 4-step preprocessing for validation
                pil_img = Image.open(image_path).convert("RGB")
                pil_img = preprocess_exact_4steps_pil(pil_img)
...
def main():
    ...
    print("=" * 60)
    print("STARTING INVENTORY DATASET MULTI-STAGE TRAINING (BEST MODEL BY VALIDATION CER)")
    print("=" * 60)
