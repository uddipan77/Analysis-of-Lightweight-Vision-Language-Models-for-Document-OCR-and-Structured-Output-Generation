#!/usr/bin/env python3
# staircase_qwen_finetune_schema_prompt_cer_multi_stage.py
# Qwen2.5-VL-7B Staircase OCR fine-tuning with Unsloth
# - Compact schema-style prompt (structure is in labels)
# - Multi-stage training (warm-up + CER-based main training)
...
class StaircaseOCRFinetune:
    ...
    # --------------------- Training (multi-stage, best model = lowest CER) --------------------- #
    ...
    def train_model(
        ...
        stage1_epochs: int = 5,   # NEW: warm-up epochs
    ):
        """
        Multi-stage training for Qwen staircase:
        - Stage 1: warm-up, teacher forcing, no eval, higher LR
        - Stage 2: main training, eval each epoch, CER-based best model via QwenCERCallback
        """
        ...
        # Directory where best-by-CER state_dict will be stored
        best_ckpt_dir = os.path.join(output_dir, "best_model_cer")

        # ==================================================================
        # STAGE 1 – Warm-up training (no eval, no CER, higher LR)
        # ==================================================================
        print("\n" + "=" * 80)
        print("STAGE 1: WARM-UP TRAINING (no eval, teacher forcing, higher LR)")
        print("=" * 80)
        ...
        stage1_args = SFTConfig(
            ...
            learning_rate=learning_rate * 3.0,  # higher LR for warm-up
            logging_steps=10,
        ...
        # Free some references & clear cache
        del trainer_stage1
        ...
        # CER-based validation callback (autoregressive decoding on val subset)
        ...
        # GPU info (for Stage 2)
        gpu_stats = torch.cuda.get_device_properties(0)
        start_gpu_memory = round(torch.cuda.max_memory_reserved() / 1024**3, 3)
        max_memory = round(gpu_stats.total_memory / 1024**3, 3)
        print(f"GPU = {gpu_stats.name}. Max memory = {max_memory} GB.")
        print(f"{start_gpu_memory} GB of memory reserved before Stage 2.")

        print("🚀 Starting Stage 2 training (best model tracked by CER callback)...\n")
        ...
        print(f"{trainer_stats.metrics['train_runtime']} seconds used for Stage 2 training.")
        print(
            f"{round(trainer_stats.metrics['train_runtime']/60, 2)} minutes used for Stage 2 training."
        )
        ...
        # ------------------------------------------------------------------
        # Load best-by-CER state_dict (saved by QwenCERCallback in Stage 2)
        # ------------------------------------------------------------------
        if cer_callback.best_state_path is not None:
            ...
            trainer_stage2.model = self.model  # keep Trainer reference in sync
            print(
            ...
            print(
                "WARNING: CER callback did not save any best model; using final Stage 2 weights."
            )
...
def main():
    base_checkpoint_dir = "/home/vault/iwi5/iwi5298h/models_image_text/qwen/stair_multi"
    ...
    config = {
        ...
        "output_dir": run_dir,
        "num_epochs": 20,
        "stage1_epochs": 5,          # NEW: warm-up epochs for Stage 1
        "batch_size": 1,
        "learning_rate": 5e-5,
        "augment_factor": 1,
    }
    ...
    try:
        print("=" * 60)
        print("STARTING STAIRCASE DATASET TRAINING (MULTI-STAGE, BEST MODEL BY VALIDATION CER)")
        print("=" * 60)
